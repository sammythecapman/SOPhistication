...
- **v5** — added `GuarantyAmount` disambiguation rule, 2026-05-04. Earlier prompts gave no field-specific guidance for `GuarantyAmountShort`/`GuarantyAmountLong`, so the model picked the most prominent guaranty-related dollar figure in the document. On 7(a) Standard loans this was almost always the **guaranteed portion** of the loan (e.g. `LoanAmount × 0.75`), not the SBA Guaranty Fee that the firm actually wants in this field. v5 adds a CRITICAL FIELD RULE clarifying that `GuarantyAmount` is the one-time upfront SBA Guaranty Fee, lists its likely phrasings ("SBA Guaranty Fee", "Guaranty Fee", "SBA Fee"), enumerates the three lookalikes the model must NOT confuse it with, and instructs the model to leave the field empty rather than computing the fee itself when only the guarantee percentage is stated.

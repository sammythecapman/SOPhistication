...
The `field_extraction` v2 prompt requires Claude to return a paired `{Field}_source` quote for every value. The pipeline verifies each quote with a two-tier check (both whitespace-collapsed and case-insensitive, ellipsis-aware for truncated quotes): **Tier 1** asks whether the literal quote is a substring of source text, and **Tier 2** falls back to checking whether the extracted value itself is a substring (handles PDF column-alignment / multi-space artifacts where the model paraphrases the citation). Results are stored per-extraction in the `field_sources` JSONB column (`{ quote, verified, quote_verified }` per field) and rendered under each field card in the UI: verified quotes get an italic monospace "Source: …" line with a terracotta border-left, fabricated quotes get a red "Unverified quote" badge, and regex-fallback fields get a small gray "Pattern-matched" pill (the literal `[regex_fallback]` sentinel is never user-visible). Analytics aggregates an "Unverified Quote Rate" across all extractions. Legacy rows render with no citations — fully backward-compatible.

## Confidence Scoring with Citation-aware YELLOW Promotion

Tiered confidence scoring (`extraction/confidence.py`) is applied to PERSON/ORG name fields only. The base rule is unchanged: NER hit → GREEN, no NER hit but value found in source text → YELLOW, value not in source text → RED. On top of that, when a YELLOW field has a verified text citation from the model — specifically `quote_verified=True`, meaning the model's literal quote was found via the strict Tier-1 substring check (NOT the looser Tier-2 value-fallback, which would be circular evidence) — the field is promoted to GREEN with `match_details` annotated `(auto-confirmed: model quoted a verbatim source string)`. This was added because empirical reviewer feedback showed every YELLOW flag was a false positive caused by `en_core_web_sm` missing uncommon surnames and small lender brand names; the model's verified verbatim citation is a stronger provenance signal than NER coverage. RED is intentionally never promoted by this rule (any value missing entirely from source text remains a hallucination risk). Sentinel quotes (`[regex_fallback]`, `[deal_analysis]`) cannot trigger promotion. Historical extractions stored before this change lack the `quote_verified` field in their JSONB blob, so `dict.get("quote_verified", False)` defaults to False and existing rows are not retroactively promoted — the change applies to new extractions only.
